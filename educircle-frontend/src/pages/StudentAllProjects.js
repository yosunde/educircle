...
const StudentAllProjects = ({ theme = "light", toggleTheme }) => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      try {
        ...
        const res = await fetch("/api/projects/student", {
    ...
    <div>
      ...
      <div style={{ display: "flex" }}>
        ...
        <div style={{ flex: 1, background: theme === "dark" ? "#23272f" : "#eaf4e6", minHeight: "100vh", padding: "32px 48px" }}>
          <h1 style={{ fontWeight: "bold", fontSize: "2rem", marginBottom: 32 }}>My Projects</h1>
          {loading ? (
