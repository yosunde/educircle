...
const StudentAllGroups = ({ theme = "light", toggleTheme }) => {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      ...
      try {
        ...
        const res = await fetch("/api/groups/student/all", {
    ...
    <div>
      ...
      <div style={{ display: "flex" }}>
        ...
        <div style={{ flex: 1, background: theme === "dark" ? "#23272f" : "#eaf4e6", minHeight: "100vh", padding: "32px 48px" }}>
          <h1 style={{ fontWeight: "bold", fontSize: "2rem", marginBottom: 32 }}>My Groups</h1>
          {loading ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              {groups.map(group => (
                <div key={group.group_id} style={{
                  ...
                  cursor: "pointer"
                }} onClick={() => navigate(`/groups/${group.group_id}/student-detail`)}>
                  <div>
